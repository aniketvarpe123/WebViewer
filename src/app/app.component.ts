...
async generateAuthenticatedFileBlob(CSConfig: any): Promise<Blob> {
  console.log('AuthToken:', CSConfig.Otcsticket);
  const url = `${CSConfig.CSUrl}/api/v1/nodes/3161277/content?Otcsticket=${CSConfig.Otcsticket}`;
  ...
  if (!this.isDocumentLoaded) {
    ...
    try {
      ...
      // Check if the response is defined and its type is a PDF
      if (response && response.type === 'application/pdf') {
        console.log('Fetched content is a PDF');
      } else if (response && response.type === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') {
        console.log('Fetched content is a DOCX');
        // Convert DOCX to PDF
        const docxBlobfile = new Blob([response], { type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' });
       // const respo= this.createBlobUrl(response);
        const pdfBlob = await this.convertDocxToPdf(docxBlobfile);
        console.log('Converted DOCX to PDF');
        return pdfBlob;
      } else {
        console.log('Fetched content is of unknown type');
      }
...
try {
    ...
    // Perform the conversion with the ArrayBuffer
    const pdfBuffer = await PDFNet.Convert.office2PDFBuffer(arrayBuffer);
  ...
  addVersion(): void {
    if (this.wvInstance) {
      ...
      annotationManager.exportAnnotations().then(async (xfdfString) => {
        ...
        const nodeId = '3161277'; // Replace with the actual nodeId
        ...
        this.addVersionToContentServer(nodeId, originalFile, uint8ArrayData);
      });
    }
  }
